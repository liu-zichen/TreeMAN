...
from conf import MIMIC_DIR
import datasets

def all_metrics(yhat, y, k=5, yhat_raw=None, calc_auc=True):
    """
        Inputs:
...
def recall_at_k(yhat_raw, y, k):
    ...
    sortd = np.argsort(yhat_raw)[:,::-1] # idx of sorted
    topk = sortd[:,:k]
...
def results_by_type(Y, mdir, version='mimic3'):
    ...
    test_file = '%s/test_%s.csv' % (MIMIC_DIR, str(Y)) if version == 'mimic3' else '%s/test.csv' % MIMIC_DIR
...
def metric2(y, y_pred):
    exp = Experiments([y, None, None], [y_pred, None, None])
    exp.metrics([1, 0, 0], avg='macro')
    exp.metrics([1, 0, 0], avg='micro')
...
if __name__ == "__main__":
    yhat_raw = np.array([[0.51, 0.2, 0.33], [0.2, 0.51, 0.3]])
    yhat = yhat_raw > 0.5
    y = np.array([[True, False, False], [True, False, False]])
    all_metrics(yhat, y, yhat_raw=yhat_raw)
